...
/*unsigned int rmultinomial(const double* p, int n, double tot_p){
	int i;
	unsigned int test[n];
	gsl_ran_multinomial (RANDOM_NUMBER,n,1,p, test);
        for (i = 0; i < n; i ++) 
		if(test[i]==1)
			break;
	return i;
...
{
    return gsl_ran_gamma(RANDOM_NUMBER, a, b);
}
...
void rdirichlet(size_t K, const double alpha[], double theta[])
{
        gsl_ran_dirichlet(RANDOM_NUMBER, K, alpha, theta);
}
